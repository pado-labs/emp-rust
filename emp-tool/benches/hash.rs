...
use emp_tool::{
    hash::{CcrHash, CrHash, Hash, TccrHash},
    Block,
};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("hash sha256", move |bench| {
        let msg: [u8; 32] = rand::random();
        bench.iter(|| black_box(Hash::new().hash_bytes_once(&msg)));
    });

    c.bench_function("hash blake3", move |bench| {
        let msg: [u8; 32] = rand::random();
        bench.iter(|| black_box(blake3::hash(&msg)));
    });
}
